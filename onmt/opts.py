...
def model_opts(parser):
    ...
    group.add('--layers', '-layers', type=int, default=12,
...
def preprocess_opts(parser):
    ...
    group.add('--train_src', '-train_src', default=["data/src-train-token.txt"], nargs='+', #required=True
              help="Path(s) to the training source data")
    group.add('--train_tgt', '-train_tgt', default=["data/tgt-train-token.txt"],  nargs='+', #required=True,
    ...
    group.add('--valid_src', '-valid_src', default="data/src-valid-token.txt",
              help="Path to the validation source data")
    group.add('--valid_tgt', '-valid_tgt', default="data/tgt-valid-token.txt",
...
def translate_opts(parser):
    ...
    group.add('--src', '-src', default="data/demo/KoreanTokenInput.txt", #data/src-test-token.txt",
              help="Source sequence to decode (one line per "
                   "sequence)")
    group.add('--src_dir', '-src_dir', default="",
              help='Source directory for image or audio files')
    group.add('--tgt', '-tgt',#data/tgt-test-token.txt",
    ...
    group.add('--shard_size', '-shard_size', type=int, default=10000,
              help="Divide src and tgt (if applicable) into "
                   ...
                   "shard_size>0 means segment dataset into multiple shards, "
                   "each shard has shard_size samples")
    group.add('--output', '-output', default='data/demo/EnglishTokenOutput.txt', #data/prediction/pred.txt',
